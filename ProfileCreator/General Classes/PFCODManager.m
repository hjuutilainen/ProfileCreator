...
- (ODSession *)newSession:(NSError **)error {
    ...
    if ( [odAddress length] != 0 && [odPort length] != 0 ) {
        return [ODSession sessionWithOptions:@{ ODSessionProxyAddress : odAddress,
                                                ODSessionProxyPort    : odPort,
                                                ODSessionProxyUsername : [ud objectForKey:@"ODUsername"] ?: @"",
                                                ODSessionProxyPassword : [ud objectForKey:@"ODPassword"] ?: @""} error:error];
    } else {
